...
{
public:
  BP_Scene(void *client_data, BP_Callback beginOverlap, BP_Callback endOverlap) :
      m_client_data(client_data), m_beginOverlap(beginOverlap), m_endOverlap(endOverlap)
  {
  }

  ~BP_Scene();

  BP_Proxy *createProxy(void *object, const Ogre::Vector3& min, const Ogre::Vector3& max);
