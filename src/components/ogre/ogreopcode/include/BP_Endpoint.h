...
public:
  ...
  BP_Endpoint(Ogre::Real pos, Type type, BP_Proxy *proxy, GEN_List& endpointList);
  ~BP_Endpoint();

  Ogre::Real getPos() const
  {
    return m_pos;
  }
  void setPos(Ogre::Real pos)
  {
    m_pos = pos;
  }
  ...
  friend void encounters(const BP_Endpoint& a, const BP_Endpoint& b, BP_Scene& scene, T_Overlap overlap);
