...
public:
  ...
  enum
  {
    CLIP_LEFT = (1 << 0), CLIP_RIGHT = (1 << 1), CLIP_BOTTOM = (1 << 2), CLIP_TOP = (1 << 3), CLIP_NEAR = (1 << 4), CLIP_FAR = (1 << 5),
  };

  enum
  {
    OUTSIDE = 0, ISEQUAL = (1 << 0), ISCONTAINED = (1 << 1), CONTAINS = (1 << 2), CLIPS = (1 << 3),
  };
  ...
  void getCenter(Ogre::Vector3& v) const
  {
    v = (vmin + vmax) * 0.5;
  }
  ...
  void getExtents(Ogre::Vector3& v) const
  {
    v = (vmax - vmin) * 0.5;
  }
  ...
  {
    if ((p.y >= vmin.y) && (p.y <= vmax.y) && (p.z >= vmin.z) && (p.z <= vmax.z))
      return true;
    else
      return false;
  }
  ...
  {
    if ((p.x >= vmin.x) && (p.x <= vmax.x) && (p.z >= vmin.z) && (p.z <= vmax.z))
      return true;
    else
      return false;
  }
  ...
  {
    if ((p.x >= vmin.x) && (p.x <= vmax.x) && (p.y >= vmin.y) && (p.y <= vmax.y))
      return true;
    else
      return false;
  }
  ...
  bool intersect(const line3& line) const
  {
    ...
    if (((s.x >= vmin.x) && (s.y >= vmin.y) && (s.z >= vmin.z) && (s.x <= vmax.x) && (s.y <= vmax.y) && (s.z <= vmax.z)) || ((e.x >= vmin.x) && (e.y >= vmin.y) && (e.z >= vmin.z) && (e.x <= vmax.x) && (e.y <= vmax.y) && (e.z <= vmax.z))) {
      return true;
    } else {
  ...
  bool intersect(const line3& line, Ogre::Vector3& ipos) const
  {
    ...
    if (line.b.x >= vmin.x && line.b.y >= vmin.y && line.b.z >= vmin.z && line.b.x <= vmax.x && line.b.y <= vmax.y && line.b.z <= vmax.z) {
      ipos = line.b;
      return true;
    }
...
public:
  ...
  inline void setStart(const Ogre::Vector3& s)
  {
    _start = s;
  }

  /// Set the end of the segment.
  inline void setEnd(const Ogre::Vector3& e)
  {
    _end = e;
  }

  /// Get the start of the segment.
  inline const Ogre::Vector3& start() const
  {
    return _start;
  }

  /// Get the end of the segment.
  inline const Ogre::Vector3& end() const
  {
    return _end;
  }

  /// Get the start of the segment.
  inline Ogre::Vector3& start()
  {
    return _start;
  }

  /// Get the end of the segment.
  inline Ogre::Vector3& end()
  {
    return _end;
  }
};
...
public:
   ...
   */
  static bool triangleTriangle(const Ogre::Vector3 tri1[3], const Ogre::Vector3 tri2[3]);
   ...
   * \return true if the triangles intersect, otherwise false
   */
  static bool triangleTriangle(const Ogre::Vector3 tri1[3], const Ogre::Vector3 tri2[3], Segment3& isectline, bool& coplanar);
