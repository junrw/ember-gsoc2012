...
public:
  ...
  inline_
  Matrix4x4& Set(float m00, float m01, float m02, float m10, float m11, float m12, float m20, float m21, float m22)
  {
    m[0][0] = m00;
    m[0][1] = m01;
    m[0][2] = m02;
  ...
  //! Assign values
  inline_
  Matrix4x4& Set(float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13, float m20, float m21, float m22, float m23, float m30, float m31, float m32, float m33)
  {
    m[0][0] = m00;
    m[0][1] = m01;
    m[0][2] = m02;
  ...
  //! Returns a row.
  inline_
  void GetRow(const udword r, HPoint& p) const
  {
    p.x = m[r][0];
    p.y = m[r][1];
    p.z = m[r][2];
  ...
  //! Returns a row.
  inline_
  void GetRow(const udword r, Point& p) const
  {
    p.x = m[r][0];
    p.y = m[r][1];
    p.z = m[r][2];
  ...
  //! Returns a row.
  inline_
  const HPoint& GetRow(const udword r) const
  {
    return *(const HPoint*)&m[r][0];
  }
  //! Returns a row.
  inline_
  HPoint& GetRow(const udword r)
  {
    return *(HPoint*)&m[r][0];
  }
  //! Sets a row.
  inline_
  void SetRow(const udword r, const HPoint& p)
  {
    m[r][0] = p.x;
    m[r][1] = p.y;
    m[r][2] = p.z;
  ...
  //! Sets a row.
  inline_
  void SetRow(const udword r, const Point& p)
  {
    m[r][0] = p.x;
    m[r][1] = p.y;
    m[r][2] = p.z;
  ...
  //! Returns a column.
  inline_
  void GetCol(const udword c, HPoint& p) const
  {
    p.x = m[0][c];
    p.y = m[1][c];
    p.z = m[2][c];
  ...
  //! Returns a column.
  inline_
  void GetCol(const udword c, Point& p) const
  {
    p.x = m[0][c];
    p.y = m[1][c];
    p.z = m[2][c];
  ...
  //! Sets a column.
  inline_
  void SetCol(const udword c, const HPoint& p)
  {
    m[0][c] = p.x;
    m[1][c] = p.y;
    m[2][c] = p.z;
  ...
  //! Sets a column.
  inline_
  void SetCol(const udword c, const Point& p)
  {
    m[0][c] = p.x;
    m[1][c] = p.y;
    m[2][c] = p.z;
  ...
  inline_
  const HPoint& GetTrans() const
  {
    return GetRow(3);
  }
  //! Gets the translation part of the matrix
  inline_
  void GetTrans(Point& p) const
  {
    p.x = m[3][0];
    p.y = m[3][1];
    p.z = m[3][2];
...
//				Matrix&	ComputeAxisMatrix(Point& axis, float angle);
  ...
  {
    return Matrix4x4(m[0][0] + mat.m[0][0], m[0][1] + mat.m[0][1], m[0][2] + mat.m[0][2], m[0][3] + mat.m[0][3], m[1][0] + mat.m[1][0], m[1][1] + mat.m[1][1], m[1][2] + mat.m[1][2], m[1][3] + mat.m[1][3], m[2][0] + mat.m[2][0], m[2][1] + mat.m[2][1], m[2][2] + mat.m[2][2], m[2][3] + mat.m[2][3], m[3][0] + mat.m[3][0], m[3][1] + mat.m[3][1], m[3][2] + mat.m[3][2], m[3][3] + mat.m[3][3]);
  }
  ...
  inline_
  HPoint operator*(const HPoint& v) const
  {
  ...
  inline_
  const HPoint& operator[](int row) const
  {
    return *(const HPoint*)&m[row][0];
  }
  inline_
  HPoint& operator[](int row)
  {
