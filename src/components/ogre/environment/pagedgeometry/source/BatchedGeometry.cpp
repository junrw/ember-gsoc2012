...
BatchedGeometry::BatchedGeometry(SceneManager *mgr, SceneNode *rootSceneNode) :
    boundsUndefined(true), sceneMgr(mgr), sceneNode(NULL), parentSceneNode(rootSceneNode), minDistanceSquared(0), withinFarDistance(0), built(false)
{
  clear();
}
...
uint32 CountUsedVertices(IndexData *id, std::map<uint32, uint32> &ibmap)
{
  ...
  {
    uint16 *data = (uint16*)id->indexBuffer->lock(id->indexStart * sizeof(uint16), id->indexCount * sizeof(uint16), HardwareBuffer::HBL_READ_ONLY);
  ...
  {
    uint32 *data = (uint32*)id->indexBuffer->lock(id->indexStart * sizeof(uint32), id->indexCount * sizeof(uint32), HardwareBuffer::HBL_READ_ONLY);
...
void BatchedGeometry::extractVertexDataFromShared(MeshPtr mesh)
{
  ...
  while (subMeshIterator.hasMoreElements()) {
    ...
    for (uint32 bufferIndex = 0; bufferIndex < buffersCount; bufferIndex++) {
      ...
      HardwareVertexBufferSharedPtr newVertexBuffer = HardwareBufferManager::getSingleton().createVertexBuffer(vertexSize, newVertexData->vertexCount, oldVertexBuffer->getUsage(), oldVertexBuffer->hasShadowBuffer());
    ...
    case HardwareIndexBuffer::IT_16BIT:
    {
      uint16 *data = (uint16*)indexData->indexBuffer->lock(indexData->indexStart * sizeof(uint16), indexData->indexCount * sizeof(uint16), HardwareBuffer::HBL_NORMAL);
    ...
    case HardwareIndexBuffer::IT_32BIT:
    {
      uint32 *data = (uint32*)indexData->indexBuffer->lock(indexData->indexStart * sizeof(uint32), indexData->indexCount * sizeof(uint32), HardwareBuffer::HBL_NORMAL);
...
BatchedGeometry::SubBatch::SubBatch(BatchedGeometry *parent, SubEntity *ent)
{
  ...
  if (blendIndices && blendWeights) {
    ...
    assert(blendIndices->getSource() == blendWeights->getSource() && "Blend indices and weights should be in the same buffer");
    assert(blendIndices->getSize() + blendWeights->getSize() == vertexData->vertexBufferBinding->getBuffer(blendIndices->getSource())->getVertexSize() && "Blend indices and blend buffers should have buffer to themselves!");
...
void BatchedGeometry::SubBatch::build()
{
  ...
  indexData->indexBuffer = HardwareBufferManager::getSingleton().createIndexBuffer(destIndexType, indexData->indexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
  ...
  for (Ogre::ushort i = 0; i < vertBinding->getBufferCount(); ++i) {
    HardwareVertexBufferSharedPtr buffer = HardwareBufferManager::getSingleton().createVertexBuffer(vertDecl->getVertexSize(i), vertexData->vertexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
  ...
  for (it = meshQueue.begin(); it != meshQueue.end(); ++it) {
    ...
    if (srcIndexType == HardwareIndexBuffer::IT_32BIT) {
      //Lock the input buffer
      uint32 *source = static_cast<uint32*>(sourceIndexData->indexBuffer->lock(sourceIndexData->indexStart, sourceIndexData->indexCount, HardwareBuffer::HBL_READ_ONLY));
      ...
      if (destIndexType == HardwareIndexBuffer::IT_32BIT) {
        ...
        //Lock the input buffer
        uint16 *source = static_cast<uint16*>(sourceIndexData->indexBuffer->lock(sourceIndexData->indexStart, sourceIndexData->indexCount, HardwareBuffer::HBL_READ_ONLY));
