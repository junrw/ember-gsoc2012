    ...
    if (HighLevelGpuProgramManager::getSingleton().getByName(vertexProgName).isNull()) {
      ...
      if (!shaderLanguage.compare("hlsl") || !shaderLanguage.compare("cg")) {
        ...
        for (unsigned short i = 0; i < subBatch->vertexData->vertexDeclaration->getElementCount(); ++i) {
          ...
          if (el->getSemantic() == VES_TEXTURE_COORDINATES) {
            if (el->getIndex() == texCoordCount - 2) {
              vertexProgSource += "	float4 params 	: TEXCOORD" + StringConverter::toString(texCoordCount - 2) + ", \n";
            } else {
              if (el->getIndex() == texCoordCount - 1) {
                vertexProgSource += "	float4 originPos 	: TEXCOORD" + StringConverter::toString(texCoordCount - 1) + ", \n";
              } else {
                String uvType = "";
                switch (el->getType()) {
                case VET_FLOAT1:
        ...
        if (fadeEnabled) {
          ...
          vertexProgSource += "	float dist = distance(camPos.xz, iPosition.xz); \n"
      ...
      if (!shaderLanguage.compare("glsl")) {
        ...
        for (unsigned short i = 0; i < subBatch->vertexData->vertexDeclaration->getElementCount(); ++i) {
          ...
          if (el->getSemantic() == VES_TEXTURE_COORDINATES) {
            if (el->getIndex() == texCoordCount - 2) {
              vertexProgSource += "	vec4 params = gl_MultiTexCoord" + StringConverter::toString(texCoordCount - 2) + "; \n";
            } else {
              if (el->getIndex() == texCoordCount - 1) {
                vertexProgSource += "	vec4 originPos = gl_MultiTexCoord" + StringConverter::toString(texCoordCount - 1) + "; \n";
              } else {
                vertexProgSource += "	gl_TexCoord[" + StringConverter::toString(texNum) + "]	= gl_MultiTexCoord" + StringConverter::toString(texNum) + "; \n";
              }
              ++texNum;
            }
          }
        ...
        if (fadeEnabled) {
          //Fade out in the distance
          vertexProgSource += "	float dist = distance(camPos.xz, gl_Vertex.xz);	\n"
      ...
      HighLevelGpuProgramPtr vertexShader = HighLevelGpuProgramManager::getSingleton().createProgram(vertexProgName, ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, shaderLanguage, GPT_VERTEX_PROGRAM);
