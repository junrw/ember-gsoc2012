...
bool Water::isSupported() const
{
  ...
  if (!caps->hasCapability(RSC_VERTEX_PROGRAM) || !(caps->hasCapability(RSC_FRAGMENT_PROGRAM))) {
    ...
    if (!GpuProgramManager::getSingleton().isSyntaxSupported("arbfp1") && !GpuProgramManager::getSingleton().isSyntaxSupported("ps_2_0") && !GpuProgramManager::getSingleton().isSyntaxSupported("ps_1_4")) {
      return false;
    }
  }
...
bool Water::initialize()
{
  try {
    ...
    /*		waterPlane.normal = ;
     waterPlane.d = 0; */
    Ogre::MeshManager::getSingleton().createPlane("WaterPlane", "environment", waterPlane, 10000, 10000, 5, 5, true, 1, 1000, 1000, Ogre::Vector3::UNIT_Z);
