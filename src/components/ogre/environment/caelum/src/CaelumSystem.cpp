...
CaelumSystem::CaelumSystem(Ogre::Root *root, Ogre::SceneManager *sceneMgr, CaelumComponent componentsToCreate/* = CAELUM_COMPONENTS_DEFAULT*/
) :
    mOgreRoot(root), mSceneMgr(sceneMgr), mCleanup(false), mAutoMoveCameraNode(true), mAutoNotifyCameraChanged(true), mAutoAttachViewportsToComponents(true), mAutoViewportBackground(true)
{
  ...
  if (std::find(groups.begin(), groups.end(), Caelum::RESOURCE_GROUP_NAME) == groups.end()) {
    LogManager::getSingleton().logMessage("Caelum: Creating required internal resource group \'" + RESOURCE_GROUP_NAME + "\'");
...
)
{
  ...
  if (componentsToCreate & CAELUM_COMPONENT_SKY_DOME) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize skydome: " + ex.getFullDescription());
    }
  }
  ...
  if (componentsToCreate & CAELUM_COMPONENT_MOON) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize moon: " + ex.getFullDescription());
    }
  }
  if (componentsToCreate & CAELUM_COMPONENT_IMAGE_STARFIELD) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize the old image starfield: " + ex.getFullDescription());
    }
  }
  if (componentsToCreate & CAELUM_COMPONENT_POINT_STARFIELD) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize starfield: " + ex.getFullDescription());
    }
  }
  if (componentsToCreate & CAELUM_COMPONENT_GROUND_FOG) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize ground fog: " + ex.getFullDescription());
    }
  }
  if (componentsToCreate & CAELUM_COMPONENT_CLOUDS) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize clouds: " + ex.getFullDescription());
    }
  }
  if (componentsToCreate & CAELUM_COMPONENT_PRECIPITATION) {
    try {
      ...
      LogManager::getSingleton().logMessage("Caelum: Failed to initialize precipitation: " + ex.getFullDescription());
    }
  }
...
{
  LogManager::getSingleton().getDefaultLog()->logMessage("CaelumSystem: Attached to"
      " viewport " + StringConverter::toString((long)vp) + " render target " + vp->getTarget()->getName());
...
{
  LogManager::getSingleton().getDefaultLog()->logMessage("CaelumSystem: Detached from "
      " viewport " + StringConverter::toString((long)vp) + " render target " + vp->getTarget()->getName());
  if (getAutoAttachViewportsToComponents()) {
    if (getPrecipitationController()) {
...
void CaelumSystem::setPrecipitationController(PrecipitationController* newptr)
{
  ...
  if (getAutoAttachViewportsToComponents() && oldptr) {
    std::for_each(mAttachedViewports.begin(), mAttachedViewports.end(), std::bind1st(std::mem_fun(&PrecipitationController::destroyViewportInstance), oldptr));
  }
  // Attach new.
  if (getAutoAttachViewportsToComponents() && newptr) {
    std::for_each(mAttachedViewports.begin(), mAttachedViewports.end(), std::bind1st(std::mem_fun(&PrecipitationController::createViewportInstance), newptr));
  }
...
void CaelumSystem::setDepthComposer(DepthComposer* ptr)
{
  ...
  if (getAutoAttachViewportsToComponents() && getDepthComposer()) {
    std::for_each(mAttachedViewports.begin(), mAttachedViewports.end(), std::bind1st(std::mem_fun(&DepthComposer::createViewportInstance), getDepthComposer()));
  }
}
...
void CaelumSystem::updateSubcomponents(Real timeSinceLastFrame)
{
  ...
  if (getManageSceneFog()) {
    mSceneMgr->setFog(Ogre::FOG_EXP2, fogColour * mSceneFogColourMultiplier, fogDensity * mSceneFogDensityMultiplier);
  }
  ...
  // Update clouds
  if (getCloudSystem()) {
    getCloudSystem()->update(secondDiff, sunDir, sunLightColour, fogColour, sunSphereColour);
  }
...
const Ogre::Vector3 CaelumSystem::makeDirection(Ogre::Degree azimuth, Ogre::Degree altitude)
{
  Ogre::Vector3 res;
...
const Ogre::Vector3 CaelumSystem::getSunDirection(LongReal jday)
{
  Ogre::Degree azimuth, altitude;
  {
    ...
    Astronomy::getHorizontalSunPosition(jday, getObserverLongitude(), getObserverLatitude(), azimuth, altitude);
  }
...
const Ogre::Vector3 CaelumSystem::getMoonDirection(LongReal jday)
{
  Ogre::Degree azimuth, altitude;
  {
    ...
    Astronomy::getHorizontalMoonPosition(jday, getObserverLongitude(), getObserverLatitude(), azimuth, altitude);
  }
  Ogre::Vector3 res = makeDirection(azimuth, altitude);
