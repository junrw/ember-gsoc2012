...
Decal::Decal(Hydrax *h, const Ogre::String &TextureName, const int& Id) :
    mHydrax(h), mTextureName(TextureName), mId(Id), mRegisteredPass(0), mPosition(Ogre::Vector2(0, 0)), mSize(Ogre::Vector2(1, 1)), mOrientation(Ogre::Radian(0)), mTransparency(1), mVisible(true)
{
...
void Decal::setTransparency(const Ogre::Real& Transparency)
{
  ...
  if (mRegisteredPass) {
    mRegisteredPass->getTextureUnitState(0)->setAlphaOperation(Ogre::LBX_MODULATE, Ogre::LBS_TEXTURE, Ogre::LBS_MANUAL, 1.0, mTransparency);
  }
}
...
{
  if (mHydrax->getCamera()->getDerivedPosition() == mLastPosition && mHydrax->getCamera()->getDerivedOrientation() == mLastOrientation) {
    return;
  }
    ...
    for (DecalIt = mDecals.begin(); DecalIt != mDecals.end(); DecalIt++) {
      ...
      if ((*DecalIt)->getRegisteredPass()) {
        ...
        if (!mHydrax->_isCurrentFrameUnderwater()) {
          (*DecalIt)->registerPass(mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_WATER)->getTechnique(0)->createPass());
        } else {
          (*DecalIt)->registerPass(mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_UNDERWATER)->getTechnique(0)->createPass());
        }
      }
  ...
  for (DecalIt = mDecals.begin(); DecalIt != mDecals.end(); DecalIt++) {
    ...
    DecalBox = Ogre::AxisAlignedBox(DPos.x - DSize.x, HHeight - mWaterStrength, DPos.y - DSize.y, DPos.x + DSize.x, HHeight + mWaterStrength, DPos.y + DSize.y);
...
Decal* DecalsManager::add(const Ogre::String& TextureName)
{
  ...
  if (mHydrax->getMaterialManager()->isCreated()) {
    NewDecal->registerPass(mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_WATER)->getTechnique(0)->createPass());
  }
