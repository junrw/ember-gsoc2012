...
{
  Ogre::String RttNames[6] = { _def_Hydrax_Reflection_Rtt_Name, _def_Hydrax_Refraction_Rtt_Name, _def_Hydrax_Depth_Rtt_Name, _def_Hydrax_Depth_Reflection_Rtt_Name, _def_Hydrax_API_Rtt_Name, _def_Hydrax_GPU_Normal_Map_Rtt_Name };
...
void RttManager::_initializeRtt(const RttType& Rtt, const Ogre::Vector3& PlaneNormal, const Ogre::ColourValue& BackgroundColour, const bool& RenderSky, Ogre::RenderTargetListener* RTListener, const Ogre::String& MaterialScheme, const bool& ShadowsEnabled)
{
  remove(Rtt);
  ...
  Ogre::MeshManager::getSingleton().createPlane(mRttOptions[Rtt].Name + "ClipPlane", HYDRAX_RESOURCE_GROUP, *mPlanes[Rtt], mHydrax->getMesh()->getSize().Width, mHydrax->getMesh()->getSize().Height, // PG grid.. <-- look into
  ...
  mTextures[Rtt] = Ogre::TextureManager::getSingleton().createManual(mRttOptions[Rtt].Name, HYDRAX_RESOURCE_GROUP, Ogre::TEX_TYPE_2D, TSize.Width, TSize.Height, 0, getPixelFormat(Rtt), Ogre::TU_RENDERTARGET);
...
void RttManager::CReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
{
  ...
  // Underwater
  if (mHydrax->_isCurrentFrameUnderwater() && (mHydrax->getCamera()->getPosition().y > mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y)) {
    mCameraPlaneDiff = 0;
    IsInUnderwaterError = true;
  }
  // Overwater
  else if ((!mHydrax->_isCurrentFrameUnderwater()) && (mHydrax->getCamera()->getPosition().y < mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y)) {
...
{
  if ((queueGroupId == Ogre::RENDER_QUEUE_SKIES_EARLY || queueGroupId == Ogre::RENDER_QUEUE_SKIES_LATE) && mActive) {
...
void RttManager::CDepthListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
{
  ...
  if (mHydrax->_isCurrentFrameUnderwater()) {
    mHydrax->getMesh()->getEntity()->setVisible(true);
    mHydrax->getMesh()->getEntity()->setMaterialName(mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_SIMPLE_RED)->getName());
