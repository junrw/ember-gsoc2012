...
bool MaterialManager::fillGpuProgramsToPass(Ogre::Pass* Pass, const Ogre::String GpuProgramNames[2], const ShaderMode& SM, const Ogre::String EntryPoints[2], const Ogre::String Data[2])
{
...
bool MaterialManager::createGpuProgram(const Ogre::String &Name, const ShaderMode& SM, const GpuProgram& GPUP, const Ogre::String& EntryPoint, const Ogre::String& Data)
{
  ...
  Ogre::HighLevelGpuProgramPtr HLGpuProgram = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(Name, HYDRAX_RESOURCE_GROUP, ShaderModesStr[static_cast<int>(SM)], GpuPType);
  ...
  case NM_RTT:
  {
    ...
    case SM_CG:
    {
      ...
      if (Options.NM == NM_TEXTURE) {
        FragmentProgramData += "float2 iUvNoise      : TEXCOORD" + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
        TEXCOORDNUM++;
      ...
      if (Options.NM == NM_VERTEX) {
        FragmentProgramData += "float4 iNormal       : TEXCOORD" + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
        TEXCOORDNUM++;
      }
      if (cFoam) {
        FragmentProgramData += "float4 iWorldPosition  : TEXCOORD" + Ogre::StringConverter::toString(TEXCOORDNUM) + ",\n";
      }
      ...
      if (Options.NM == NM_TEXTURE || Options.NM == NM_RTT) {
        FragmentProgramData += "uniform sampler2D    uNormalMap       : register(s" + Ogre::StringConverter::toString(TexNum) + "),\n";
        TexNum++;
      }
      ...
      if (cDepth) {
        FragmentProgramData += "uniform sampler2D    uDepthMap        : register(s" + Ogre::StringConverter::toString(TexNum) + "),\n";
        TexNum++;
      }
      ...
      FragmentProgramData += "oColor = lerp(refraction,reflection,fresnel);\n";
      ...
      if (cSmooth) {
        FragmentProgramData += "oColor.xyz = lerp(tex2D(uRefractionMap,ProjectionCoord.xy).xyz,oColor.xyz,saturate((1-tex2D(uDepthMap,ProjectionCoord.xy).r)*uSmoothPower));\n";
      }
...
bool MaterialManager::_createUnderwaterMaterial(const HydraxComponent &Components, const Options &Options)
{
  ...
  case NM_RTT:
  {
    ...
    case SM_CG:
    {
      ...
      if (cUReflections) {
        FragmentProgramData += "uniform sampler2D    uReflectionMap   : register(s" + Ogre::StringConverter::toString(TexNum) + "),\n";
        TexNum++;
      }

      FragmentProgramData += "uniform sampler2D    uRefractionMap   : register(s" + Ogre::StringConverter::toString(TexNum) + "),\n";
      TexNum++;

      if (cDepth && cUReflections) {
        FragmentProgramData += "uniform sampler2D    uDepthReflectionMap : register(s" + Ogre::StringConverter::toString(TexNum) + "),\n";
        TexNum++;
      }
      ...
      //"float fresnel = tex1D(uFresnelMap,dotProduct);\n" +
          ...
          "float4 refraction=tex2D(uRefractionMap,ProjectionCoord.xy-pixelNormalModified);\n";
      if (cUReflections) {
        FragmentProgramData += "float4 reflection=tex2D(uReflectionMap,ProjectionCoord.xy+pixelNormalModified);\n";
      } else {
        FragmentProgramData += "float4 reflection=float4(uWaterColor,1);\n";
      }
      ...
      if (cSun) {
        ...
        // Temporally solution, fix this
            "surfaceToSun.xz = -surfaceToSun.xz;" + "float3 sunlight = uSunStrength*pow(saturate(dot(refractedVector,surfaceToSun)),uSunArea)*uSunColor;\n" + "oColor.xyz+=sunlight*saturate(1-additionalReflection);\n";
      }
...
bool MaterialManager::_createUnderwaterCompositor(const HydraxComponent &Components, const Options &Options)
{
  ...
  case SM_CG:
  {
    ...
    if (cCaustics) // Depth, caustics
    {
      ...
      if (cGodRays) {
        FragmentProgramData += Ogre::String("float3 view_vector = normalize(iPosition-uCameraPos);") + "float dot_product = dot(view_vector, -uLightDirection);" + "float num = uHGg.x;" + "float den = (uHGg.y - uHGg.z*dot_product); " + "den = rsqrt(den); " + "float phase = num * (den*den*den);" + "oColor.xyz += (0.15 + uIntensity*tex2D(uDepthMap, iUV).z)*phase*uSunColor;";
      }
    } else if (cDepth) // Depth, no caustics
    {
      FragmentProgramData += "oColor = lerp(float4(uWaterColor,1),tex2D(uOriginalMap, iUV+distortUV),tex2D(uDepthMap, iUV+distortUV).r);\n";
      if (cGodRays) {
        FragmentProgramData += Ogre::String("float3 view_vector = normalize(iPosition-uCameraPos);") + "float dot_product = dot(view_vector, -uLightDirection);" + "float num = uHGg.x;" + "float den = (uHGg.y - uHGg.z*dot_product); " + "den = rsqrt(den); " + "float phase = num * (den*den*den);" + "oColor.xyz += (0.15 + uIntensity*tex2D(uDepthMap, iUV).y)*phase*uSunColor;";
      }
    } else // No depth, no caustics
    {
      FragmentProgramData += "oColor = tex2D(uOriginalMap, iUV+distortUV);";
    }
    FragmentProgramData += "}\n";
  }
    break;
  ...
  if (cGodRays) {
    FP_Parameters->setNamedConstant("uSunColor", mHydrax->getSunColor());
    FP_Parameters->setNamedConstant("uLightDirection", (mHydrax->getMesh()->getObjectSpacePosition(mHydrax->getCamera()->getPosition()) - mHydrax->getMesh()->getObjectSpacePosition(mHydrax->getSunPosition())).normalisedCopy());
  ...
  Ogre::CompositorManager::getSingleton().addCompositor(mHydrax->getViewport(), _def_Underwater_Compositor_Name)->addListener(&mUnderwaterCompositorListener);
...
void MaterialManager::UnderwaterCompositorListener::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr &mat)
{
  ...
  if (mMaterialManager->mHydrax->isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS)) {
    ...
    FP_Parameters->setNamedConstant("uLightDirection", (mMaterialManager->mHydrax->getMesh()->getObjectSpacePosition(mMaterialManager->mHydrax->getCamera()->getPosition()) - mMaterialManager->mHydrax->getMesh()->getObjectSpacePosition(mMaterialManager->mHydrax->getSunPosition())).normalisedCopy());
