...
SimpleGrid::SimpleGrid(Hydrax *h, Noise::Noise *n, const MaterialManager::NormalMode& NormalMode) :
    Module("SimpleGrid" + _SG_getNormalModeString(NormalMode), n, Mesh::Options(256, Size(mOptions.MeshSize), _SG_getVertexTypeFromNormalMode(NormalMode)), NormalMode), mHydrax(h), mVertices(0)
{
}

SimpleGrid::SimpleGrid(Hydrax *h, Noise::Noise *n, const MaterialManager::NormalMode& NormalMode, const Options &Options) :
    Module("SimpleGrid" + _SG_getNormalModeString(NormalMode), n, Mesh::Options(Options.Complexity, Size(Options.MeshSize), _SG_getVertexTypeFromNormalMode(NormalMode)), NormalMode), mHydrax(h), mVertices(0)
{
  setOptions(Options);
}
...
bool SimpleGrid::loadCfg(const Ogre::ConfigFile &CfgFile)
{
  ...
  setOptions(Options(CfgFileManager::_getIntValue(CfgFile, "SG_Complexity"), CfgFileManager::_getSizeValue(CfgFile, "SG_MeshSize"), CfgFileManager::_getFloatValue(CfgFile, "SG_Strength"), CfgFileManager::_getBoolValue(CfgFile, "PG_Smooth"), CfgFileManager::_getBoolValue(CfgFile, "PG_ChoppyWaves"), CfgFileManager::_getFloatValue(CfgFile, "PG_ChoopyStrength")));
...
void SimpleGrid::update(const Ogre::Real &timeSinceLastFrame)
{
  ...
  if (getNormalMode() == MaterialManager::NM_VERTEX) {
    ...
    for (v = 0; v < mOptions.Complexity; v++) {
      ...
      for (u = 0; u < mOptions.Complexity; u++) {
        Vertices[i].y = mNoise->getValue(v_, (static_cast<float>(u) / (mOptions.Complexity - 1)) * mOptions.MeshSize.Height) * mOptions.Strength;
        i++;
      }
    }
  } else if (getNormalMode() == MaterialManager::NM_RTT) {
  ...
  if (mOptions.Smooth) {
    if (getNormalMode() == MaterialManager::NM_VERTEX) {
      ...
      for (v = 1; v < (mOptions.Complexity - 1); v++) {
        for (u = 1; u < (mOptions.Complexity - 1); u++) {
          Vertices[v * mOptions.Complexity + u].y = 0.2f * (Vertices[v * mOptions.Complexity + u].y + Vertices[v * mOptions.Complexity + (u + 1)].y + Vertices[v * mOptions.Complexity + (u - 1)].y + Vertices[(v + 1) * mOptions.Complexity + u].y + Vertices[(v - 1) * mOptions.Complexity + u].y);
        }
      }
    } else if (getNormalMode() == MaterialManager::NM_RTT) {
      Mesh::POS_VERTEX* Vertices = static_cast<Mesh::POS_VERTEX*>(mVertices);
...
void SimpleGrid::_calculeNormals()
{
  ...
  for (v = 1; v < (mOptions.Complexity - 1); v++) {
    for (u = 1; u < (mOptions.Complexity - 1); u++) {
      vec1 = Ogre::Vector3(Vertices[v * mOptions.Complexity + u + 1].x - Vertices[v * mOptions.Complexity + u - 1].x, Vertices[v * mOptions.Complexity + u + 1].y - Vertices[v * mOptions.Complexity + u - 1].y, Vertices[v * mOptions.Complexity + u + 1].z - Vertices[v * mOptions.Complexity + u - 1].z);

      vec2 = Ogre::Vector3(Vertices[(v + 1) * mOptions.Complexity + u].x - Vertices[(v - 1) * mOptions.Complexity + u].x, Vertices[(v + 1) * mOptions.Complexity + u].y - Vertices[(v - 1) * mOptions.Complexity + u].y, Vertices[(v + 1) * mOptions.Complexity + u].z - Vertices[(v - 1) * mOptions.Complexity + u].z);
