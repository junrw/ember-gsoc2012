...
{
Perlin::Perlin() :
    Noise("Perlin", true), octaves(0), time(0), r_noise(0), magnitude(n_dec_magn * 0.085f), mGPUNormalMapManager(0)
{
}

Perlin::Perlin(const Options &Options) :
    Noise("Perlin", true), mOptions(Options), octaves(0), time(0), r_noise(0), magnitude(n_dec_magn * Options.Scale), mGPUNormalMapManager(0)
{
}
...
void Perlin::setOptions(const Options &Options)
{
  if (isCreated()) {
    ...
    if (isGPUNormalMapSupported() && areGPUNormalMapResourcesCreated()) {
      mGPUNormalMapManager->getNormalMapMaterial()->getTechnique(0)->getPass(0)->getVertexProgramParameters()->setNamedConstant("uScale", Options.Scale);

      mGPUNormalMapManager->getNormalMapMaterial()->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->setNamedConstant("uStrength", Options.GPU_Strength);

      mGPUNormalMapManager->getNormalMapMaterial()->getTechnique(0)->getPass(0)->getFragmentProgramParameters()->setNamedConstant("uLODParameters", Options.GPU_LODParameters);
    }
  } else {
    mOptions = Options;
  }
...
bool Perlin::createGPUNormalMapResources(GPUNormalMapManager *g)
{
  ...
  Ogre::TexturePtr mPerlinTexture0 = Ogre::TextureManager::getSingleton().createManual("_Hydrax_Perlin_Noise0", HYDRAX_RESOURCE_GROUP, Ogre::TEX_TYPE_2D, np_size, np_size, 0, Ogre::PF_L16, Ogre::TU_DYNAMIC_WRITE_ONLY);

  Ogre::TexturePtr mPerlinTexture1 = Ogre::TextureManager::getSingleton().createManual("_Hydrax_Perlin_Noise1", HYDRAX_RESOURCE_GROUP, Ogre::TEX_TYPE_2D, np_size, np_size, 0, Ogre::PF_L16, Ogre::TU_DYNAMIC_WRITE_ONLY);
  ...
  Technique0_Pass0->createTextureUnitState(mGPUNormalMapManager->getTexture(0)->getName(), 0)->setTextureAddressingMode(Ogre::TextureUnitState::TAM_MIRROR);
  Technique0_Pass0->createTextureUnitState(mGPUNormalMapManager->getTexture(1)->getName(), 1)->setTextureAddressingMode(Ogre::TextureUnitState::TAM_MIRROR);
  ...
  for (frame = 0; frame < noise_frames; frame++) {
    for (v = 0; v < n_size; v++) {
      for (u = 0; u < n_size; u++) {
        ...
        temp = (1.0f / 14.0f) * (tempnoise[f + v0 + u0] + tempnoise[f + v0 + u1] + tempnoise[f + v0 + u2] + tempnoise[f + v1 + u0] + 6.0f * tempnoise[f + v1 + u1] + tempnoise[f + v1 + u2] + tempnoise[f + v2 + u0] + tempnoise[f + v2 + u1] + tempnoise[f + v2 + u2]);
...
#define min(a,b) (((a) < (b)) ? (a) : (b))
  ...
  for (o = 0; o < octaves; o++) {
    ...
    for (i = 0; i < n_size_sq; i++) {
      o_noise[i + n_size_sq * o] = (((amount[0] * noise[i + n_size_sq * image[0]]) >> scale_decimalbits) + ((amount[1] * noise[i + n_size_sq * image[1]]) >> scale_decimalbits) + ((amount[2] * noise[i + n_size_sq * image[2]]) >> scale_decimalbits));
    }
...
float Perlin::_getHeigthDual(float u, float v)
{
  ...
  int ui = u * magnitude, vi = v * magnitude, i, value = 0, hoct = octaves / n_packsize;
...
int Perlin::_mapSample(const int &u, const int &v, const int &upsamplepower, const int &octave)
{
  ...
  o = fu_m * fv_m * o_noise[octave * n_size_sq + ((pv) & n_size_m1) * n_size + ((pu) & n_size_m1)] + fu * fv_m * o_noise[octave * n_size_sq + ((pv) & n_size_m1) * n_size + ((pu + 1) & n_size_m1)] + fu_m * fv * o_noise[octave * n_size_sq + ((pv + 1) & n_size_m1) * n_size + ((pu) & n_size_m1)] + fu * fv * o_noise[octave * n_size_sq + ((pv + 1) & n_size_m1) * n_size + ((pu + 1) & n_size_m1)];
