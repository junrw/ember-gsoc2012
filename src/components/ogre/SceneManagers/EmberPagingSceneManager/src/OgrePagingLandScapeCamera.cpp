...
PagingLandScapeCamera::PagingLandScapeCamera(const String& name, SceneManager* sm) :
    ...
    mCurrentCameraPageX(0), mCurrentCameraPageZ(0), mCurrentCameraTileX(0), mCurrentCameraTileZ(0), mIniX(0), mFinX(0), mIniZ(0), mFinZ(0), mPreIniX(0), mPreFinX(0), mPreIniZ(0), mPreFinZ(0), mLastCameraPos(Vector3(std::numeric_limits < Real > ::max(), 0.0f, std::numeric_limits < Real > ::max()))
{
...
void PagingLandScapeCamera::resetPaging()
{
  ...
  mPreIniZ = 0;
  mPreFinZ = 0;
  mLastCameraPos = Vector3(std::numeric_limits < Real > ::max(), 0.0f, std::numeric_limits < Real > ::max());
  updatePaging(0, 0);
...
void PagingLandScapeCamera::updatePaging(const unsigned int x, const unsigned int z)
{
  ...
  const PagingLandScapeOptions * const opt = static_cast<PagingLandScapeSceneManager*>(mSceneMgr)->getOptions();
