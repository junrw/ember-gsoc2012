...
PagingLandScapePage::PagingLandScapePage(PagingLandScapePageManager& pageMgr) :
    mPageMgr(pageMgr), mPageState(STATE_UNINITED), mPageInQueue(QUEUE_NONE), mTableX(0), mTableZ(0), mWorldPosition(Ogre::Vector3::ZERO), mVisible(false), mPageNode(0), mRenderable(0), mNumTiles(0)
{
  touch();
...
void PagingLandScapePage::init(unsigned int tableX, unsigned int tableZ)
{
  ...
  const String NodeName = "PagingLandScapePage." + StringConverter::toString(mTableX) + "." + StringConverter::toString(mTableZ) + ".Node";
  ...
  // create renderable
  if (options->BigImage) {
    mRenderable = new PagingLandScapePageRenderable(&mPageMgr, mPageNode->getName() + "rend", mTableX, mTableZ, mBounds);
...
void PagingLandScapePage::fireEvent(EventType type)
{
  ...
  {
    listenerMgr->firePagePreloaded(mTableX, mTableZ, data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
  }
    break;
  case EVENT_LOADED:
  {
    listenerMgr->firePageLoaded(mTableX, mTableZ, data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
    mPageMgr.eventPageLoaded(this);
  }
    break;
  case EVENT_UNLOADED:
  {
    listenerMgr->firePageUnloaded(mTableX, mTableZ, data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
  }
    break;
  ...
  case EVENT_SHOW:
  {
    listenerMgr->firePageShow(mTableX, mTableZ, data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
  }
    break;
  case EVENT_HIDE:
  {
    listenerMgr->firePageHide(mTableX, mTableZ, data2dMgr->getData2D(mTableX, mTableZ)->getHeightData(), mBounds);
  }
    break;
  default:
