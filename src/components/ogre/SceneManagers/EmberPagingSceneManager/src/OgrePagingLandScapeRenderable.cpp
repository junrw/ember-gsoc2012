...
PagingLandScapeRenderable::PagingLandScapeRenderable(PagingLandScapeRenderableManager *renderableMgr) :
    Renderable(), MovableObject(), mParent(renderableMgr), mCurrVertexes(0), mInfo(0), mMaterialLODIndex(0), mInUse(false), mIsLoaded(false), mDeltaBuffers(0), mLastNextLevel(-1), mLODMorphFactor(0.0f), mHeightfield(0), mMinLevelDistSqr(0), mRenderLevel(-1), mIsRectModified(false), mRect(0, 0, 0, 0, 0, 1), mForcedMaxLod(false)
    //Ember added start
...

{
  ...
  HardwareVertexBufferSharedPtr vbuf = HardwareBufferManager::getSingleton().createVertexBuffer(decl->getVertexSize(MAIN_BINDING), mCurrVertexes->vertexCount, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
  ...
  mCustomGpuParameters = Vector4(mParent->getOptions()->ScaledPageSizeX, mParent->getOptions()->ScaledHeightY, mParent->getOptions()->ScaledPageSizeZ, mLODMorphFactor);
...
void PagingLandScapeRenderable::init(PagingLandScapeTileInfo* info)
{
  ...
  mInfo = info;
  mInUse = true;
  mName = StringConverter::toString(mInfo->mPageX) + "." + StringConverter::toString(mInfo->mPageZ) + "." + StringConverter::toString(mInfo->mTileX) + "." + StringConverter::toString(mInfo->mTileZ) + "Rend";
  ...
  bind->setBinding(TEXTURE_BINDING, mParent->getSceneManager()->getTextureCoordinatesManager()->getBuffer(info->mTileX, info->mTileZ));
}
//-----------------------------------------------------------------------
...
//                +  " | " + StringConverter::toString(mRect.top) + " - " + StringConverter::toString(mRect.bottom));
  ...
  uchar* pMain = static_cast<uchar*>(vVertices->lock(VertexTileSize * mRect.top + mRect.left * VertexSize, // offset
  ...
  if (!mIsRectModified) {
    mBounds.setExtents(offSetX * scale_x, min, offSetZ * scale_z, endx * scale_x, max, endz * scale_z);
  } // if (!mIsRectModified) 
  else {
    const Vector3& maxbound(mBounds.getMaximum());
    const Vector3& minbound(mBounds.getMinimum());
    mBounds.setExtents(minbound.x, min, minbound.z, maxbound.x, max, maxbound.z);
  }
  mBoundingRadius = Math::Sqrt(Math::Sqr(max - min) + Math::Sqr((endx - 1 - offSetX) * scale_x) + Math::Sqr((endz - 1 - offSetZ) * scale_z)) / 2;
...
//         getWorldBoundingBox(true);
  if (!mIsLoaded) {
    mParent->getSceneManager()->getListenerManager()->fireTileLoaded(mInfo->mPageX, mInfo->mPageZ, mInfo->mTileX, mInfo->mTileZ, mParentTile->getWorldBbox());
    mIsLoaded = true;
  } else if (mNeedReload) {
    mParent->getSceneManager()->getListenerManager()->fireTileDeformed(mInfo->mPageX, mInfo->mPageZ, mInfo->mTileX, mInfo->mTileZ, mParentTile->getWorldBbox());
    mNeedReload = false;
  }
  mRect.left = 0;
  mRect.right = 0;
  ...
  if (mInUse && mIsLoaded && mVisible) {
    ...
    if (mParent->getOptions()->primaryCamera == cam || mRenderLevel == -1) {
      ...
      if (mForcedMaxLod) {
        ...
        // Material LOD
        if (!mMaterial.isNull() && mMaterial->getNumLodLevels(MaterialManager::getSingleton()._getActiveSchemeIndex()) > 1) {
        ...
        if (mParent->getOptions()->lodMorph) {
          ...
          if (nextLevel == 0) {
            ...
            if (range) {
              ...
              mLODMorphFactor = std::max((percent - mParent->getOptions()->lodMorphStart) * rescale, static_cast<Real>(0.0f));
              mCustomGpuParameters.w = mLODMorphFactor;
          ...
          if (mLastNextLevel != nextLevel) {
            ...
            if (nextLevel > 1) {
              mCurrVertexes->vertexBufferBinding->setBinding(DELTA_BINDING, mDeltaBuffers[nextLevel - 1]);
            } else {
              // bind dummy (in case bindings checked)
              mCurrVertexes->vertexBufferBinding->setBinding(DELTA_BINDING, mDeltaBuffers[0]);
            }
          }
...
Real PagingLandScapeRenderable::getSquaredViewDepth(const Camera* cam) const
{
  ...
  return (PagingLandScapeRenderable::getWorldPosition() - cam->getDerivedPosition()).squaredLength();
}
...
void PagingLandScapeRenderable::getRenderOperation(RenderOperation& op)
{
  ...
  if (mChangedRenderLevel) {
    mIndex = mParent->getSceneManager()->getIndexBufferManager()->getIndexData(mRenderLevel, mNeighbors);
    mChangedRenderLevel = false;
  }
  assert(mIndex);
  op.indexData = mIndex;
...
// should change if we're just refilling a part to whole tile (only if using discard, tough)
      for (i = offSetX; i < endx; i++) {
        ...
        BaseHeight[k] = static_cast<short>((heightField[i + K_heightFieldPos] * inv_scale) - 32768);
...
// should change to an interval locking (as in load) ? if we're just refilling a part to whole tile 
      pDeltaInit = static_cast<ushort*>(mDeltaBuffers[lvl_minus_one]->lock(HardwareBuffer::HBL_DISCARD));
          ...
          for (int z = 0; z <= zubound; z++) {
            ...
            const int zPageSize = z * pageSize;
            {
              ...
              for (int x = 0; x <= xubound; x++) {
                ...
                if (xpct + zpct <= 1.0f) {
                  // Solve for x/z
                  interp_h = (-(t1.normal.x * fulldetailx) - t1.normal.z * fulldetailz - t1.d) / t1.normal.y;
                } else {
                  // Second triangle
                  interp_h = (-(t2.normal.x * fulldetailx) - t2.normal.z * fulldetailz - t2.d) / t2.normal.y;
                }
                ...
                if (lodMorph) {
                  ...
                  if (delta != 0.0f) {
                    ...
                    if (tileposx != 0 && tileposx != (tilesize - 1) && tileposy != 0 && tileposy != (tilesize - 1)) {

                      assert((tileposx + (tileposy * tilesize))*blendWeights < size);

                      pDeltas[(tileposx + (tileposy * tilesize)) * blendWeights] = static_cast<short>((interp_h * inv_scale) - 32768);
...
HardwareVertexBufferSharedPtr PagingLandScapeRenderable::createDeltaBuffer(void) const
{
  ...
  HardwareVertexBufferSharedPtr buf = HardwareBufferManager::getSingleton().createVertexBuffer(VertexElement::getTypeSize(VET_SHORT2),
  //VertexElement::getTypeSize(VET_SHORT1), 
  //VertexElement::getTypeSize(VET_FLOAT1), 
      mParent->getOptions()->TileSize * mParent->getOptions()->TileSize, HardwareBuffer::HBU_STATIC_WRITE_ONLY);
  // HardwareBuffer::HBU_STATIC);
...
Real PagingLandScapeRenderable::_vertex(const int x, const int z, const int n) const
{
  ...
  case 1:
    return mHeightfield[((mInfo->mTileX * tilesize) + x) + ((mInfo->mTileZ * tilesize) + z) * mParent->getOptions()->PageSize];
  case 2:
...
//-----------------------------------------------------------------------
void PagingLandScapeRenderable::_updateCustomGpuParameter(const GpuProgramParameters::AutoConstantEntry& constantEntry, GpuProgramParameters* params) const
{
