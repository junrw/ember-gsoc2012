...
void PagingLandScapeOctreeIntersectionSceneQuery::execute(IntersectionSceneQueryListener* listener)
{
  ...
  while (factIt.hasMoreElements()) {
    SceneManager::MovableObjectIterator it = mParentSceneMgr->getMovableObjectIterator(factIt.getNext()->getType());
    while (it.hasMoreElements()) {
      ...
      while (itscn != list.end()) {
        ...
        while (oit.hasMoreElements()) {
          ...
          if (m != e && set.find(MovablePair(e, m)) == set.end() && set.find(MovablePair(m, e)) == set.end() && (m->getQueryFlags() & mQueryMask) && (m->getTypeFlags() & mQueryTypeMask) && m->isInScene() && e->getWorldBoundingBox().intersects(m->getWorldBoundingBox())) {
            ...
            if (m->getMovableType() == "Entity") {
              ...
              while (childIt.hasMoreElements()) {
                ...
                if (c->getQueryFlags() & mQueryMask && e->getWorldBoundingBox().intersects(c->getWorldBoundingBox())) {
