...
PagingLandScapeData2D::PagingLandScapeData2D(PagingLandScapeData2DManager* pageMgr) :
    mParent(pageMgr), mHeightData(0), mIsLoaded(false), mIsModified(false), mIsRectModified(false), mRect(0, 0, 0, 0, 0, 1)
{
}
...
void PagingLandScapeData2D::adjustDeformationRectangle(unsigned int x, unsigned int z)
{
  ...
  PagingLandScapeTile *t = mParent->getSceneManager()->getPageManager()->getTilePage(tileposx, tileposz, mPageX, mPageZ);
  if (t && t->isLoaded()) {
...
#define  getisIn(a, b) (mHeightData[static_cast<unsigned int> (a) + static_cast<unsigned int> (b) * mSize])
  ...
  if (mX > 0 && mZ > 0 && mX < pageSize && mZ < pageSize) {
    ...
    Vector3 result((getisIn (mX - 1 , mZ) - getisIn (mX + 1 , mZ)) * Divider, 2.0f, (getisIn (mX, mZ - 1) - getisIn (mX , mZ + 1)) * Divider);
    ...
    Vector3 result((a - b) * Divider, 2.0f, (c - d) * Divider);
    result.normalise();
    return result;
