...
{
//-----------------------------------------------------------------------
PagingLandScapeTexture::PagingLandScapeTexture(PagingLandScapeTextureManager *textureMgr, const String materialBaseName, const unsigned int numTexture, const bool isSplatMode) :
...
void PagingLandScapeTexture::bindCompressionSettings(GpuProgramParametersSharedPtr params)
{
  ...
  while (aci.hasMoreElements()) {
    ...
    if (ace.paramType == GpuProgramParameters::ACT_CUSTOM && ace.data == MORPH_CUSTOM_PARAM_ID) {
      found = true;
    }
  }
  if (!found) {
    params->setNamedAutoConstant("compressionSettings", GpuProgramParameters::ACT_CUSTOM, MORPH_CUSTOM_PARAM_ID);
  }
}
