...
//-----------------------------------------------------------------------
Ogre::SceneManager* EmberPagingSceneManagerFactory::createInstance(const Ogre::String& instanceName)
{
...
Model::Model* EmberPagingSceneManager::createModel(const Ogre::String& modelName, const Ogre::String& modelDefinitionName)
{
  // delegate to factory implementation
  Ogre::NameValuePairList params;
  params["modeldefinition"] = modelDefinitionName;
  return static_cast<Model::Model*>(createMovableObject(modelName, Model::ModelFactory::FACTORY_TYPE_NAME, &params));
...
const Ogre::Pass* EmberPagingSceneManager::deriveShadowCasterPass(const Ogre::Pass* pass)
{
  ...
  if (isShadowTechniqueTextureBased()) {
    ...
    if (!shadowCasterMaterial.isNull() && shadowCasterMaterial->getBestTechnique()) {
      ...
      retPass = mShadowTextureCustomCasterPass ? mShadowTextureCustomCasterPass : mShadowCasterPlainBlackPass;
    }

    // Special case alpha-blended passes
    if ((pass->getSourceBlendFactor() == SBF_SOURCE_ALPHA && pass->getDestBlendFactor() == SBF_ONE_MINUS_SOURCE_ALPHA) || pass->getAlphaRejectFunction() != CMPF_ALWAYS_PASS) {
      // Alpha blended passes must retain their transparency
      retPass->setAlphaRejectSettings(pass->getAlphaRejectFunction(), pass->getAlphaRejectValue());
      ...
      for (unsigned short t = 0; t < origPassTUCount; ++t) {
        ...
        tex->setColourOperationEx(LBX_SOURCE1, LBS_MANUAL, LBS_CURRENT, isShadowTechniqueAdditive() ? ColourValue::Black : mShadowColour);
    ...
    if (!pass->getShadowCasterVertexProgramName().empty()) {
      ...
      if (!customMaterial) {
        if (retPass == mShadowTextureCustomCasterPass) {
          // reset vp?
          if (mShadowTextureCustomCasterPass->getVertexProgramName() != mShadowTextureCustomCasterVertexProgram) {
            mShadowTextureCustomCasterPass->setVertexProgram(mShadowTextureCustomCasterVertexProgram, false);
            if (mShadowTextureCustomCasterPass->hasVertexProgram()) {
              mShadowTextureCustomCasterPass->setVertexProgramParameters(mShadowTextureCustomCasterVPParams);
