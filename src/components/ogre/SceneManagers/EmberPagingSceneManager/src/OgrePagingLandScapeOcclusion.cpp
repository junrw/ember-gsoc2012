...
Occlusion::Occlusion(unsigned int visibilityThsd) :
    mCurrentCam(0), mVisibilityTreshold(visibilityThsd), mIsQueryPoolNotInitiated(true), mFrameConservativeVisibility(300)
{
}
;
//-----------------------------------------------------------------------
bool Occlusion::nextFrame(PagingLandScapeOctreeCamera *cam, MovableObjectList *camInProgressVisibles, bool onlyshadowcaster, RenderQueue *q)
{
  bool newframe = true;
...
void Occlusion::CHCtraversal(PagingLandScapeOctree *octree, VisibleObjectsBoundsInfo * const visibleBounds)
{
  ...
  while (!traversalStack.empty() || !m_queryQueue.empty()) {
    ...
    while (traversalStack.empty() && !m_queryQueue.empty() && !m_queryQueue.front()->getNodeData(mCurrentCam)->query->isStillOutstanding()) {
    ...
    while (!traversalStack.empty()) {
      ...
      if (insideViewFrustum(node)) {
        ...
        const bool wasVisible = vis->queryVisible && lastframeId == vis->lastQueryFrameId;
        if (node.isLeaf()) {
  ...
  while (!traversalStack.empty() || !m_queryQueue.empty()) {
    ...
    while (!traversalStack.empty()) {
      ...
      if (insideViewFrustum(node)) {
        ...
        const bool wasVisibleandQueriedBeforeLastNFrame = wasVisible && lastframeId - frameConservativeVisibility < vis->lastQueryFrameId;
        if (wasVisibleandQueriedBeforeLastNFrame) {
          ...
          const bool wasVisibleandQueriedExacltyLastNFrame = wasVisible && lastframeId - frameConservativeVisibility == vis->lastQueryFrameId;
          if (node.isLeaf()) {
