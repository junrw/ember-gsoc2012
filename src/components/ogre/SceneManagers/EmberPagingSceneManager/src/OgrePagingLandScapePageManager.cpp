...
PagingLandScapePageManager::PagingLandScapePageManager(PagingLandScapeSceneManager* scnMgr) :
    mSceneManager(scnMgr), mOptions(scnMgr->getOptions()), mData2d(scnMgr->getData2DManager()), mTexture(scnMgr->getTextureManager()), mRenderablesMgr(scnMgr->getRenderableManager()), mWidth(0), mHeight(0), mPause(99), mCurrentcam(0), mTerrainReady(false), mRenderQueueGroupID(static_cast<Ogre::RenderQueueGroupID>(scnMgr->getWorldGeometryRenderQueue())),
...
{
  std::for_each(mActivePages.begin(), mActivePages.end(), std::mem_fun(&PagingLandScapePage::uninit));
...
{
  std::for_each(mActivePages.begin(), mActivePages.end(), std::mem_fun(&PagingLandScapePage::setMapMaterial));
}
//-----------------------------------------------------------------------
...
//-----------------------------------------------------------------------
void PagingLandScapePageManager::getNearestTileIndicesUnscaled(Real posx, Real posz, unsigned int pagex, unsigned int pagez, unsigned int& x, unsigned int& z) const
{
  // adjust x and z to be local to page
...
//-----------------------------------------------------------------------
bool PagingLandScapePageManager::getTileIndices(Real posx, Real posz, unsigned int pagex, unsigned int pagez, unsigned int& x, unsigned int& z, bool alwaysAnswer) const
{
  if (alwaysAnswer) {
