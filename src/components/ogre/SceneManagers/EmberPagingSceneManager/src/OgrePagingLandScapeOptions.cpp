...
{
  if (TextureFormatSupported.end() == std::find(TextureFormatSupported.begin(), TextureFormatSupported.end(), format)) {
...
const String &PagingLandScapeOptions::getMapFilename(const String &currMapName) const
{
  ...
  for (i = mMapList.begin(); i != iEnd; ++i) {
    if (StringUtil::match(i->second, currMapName, false) || StringUtil::match(i->first, currMapName, false)) {
      return i->second;
    }
  }
  return currMapName;
...
void PagingLandScapeOptions::loadMapOptions(DataStreamPtr &stream)
{
  ...
  if (world_width == 0 || world_height == 0) {
    OGRE_EXCEPT(1, "Your map must at least have a width and height defined in the config file ", "PagingLandScapeOptions::loadMapOptions");
  }
...
//         setReal (scale.z, "ScaleZ");
  ...
  if (HeightFieldRawTC || HeightFieldTC || HeightFieldNTC) {
  ...
  for (i = 0; i < NumMatHeightSplat; i++) {
    setReal(matHeight[i], "MaterialHeight" + StringConverter::toString(i));
    matHeight[i] *= divider;
  }

  const String baseSplatName("SplatFilename");
  for (i = 0; i < NumMatHeightSplat; i++)
    setString(SplatDetailMapNames[i], baseSplatName + StringConverter::toString(i));
  getAvgColors();
  ...
  if (coverage_vertex_color || base_vertex_color) // || vertex_shadowed || vertex_instant_colored)
    colored = true;
...
ColourValue PagingLandScapeOptions::_getAvgColor(const String &tex) const
{
  ...
  if (!data) {
    OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND, "You need to define SplatFilename that has at least 3 or 4 bytes componennts (RGB or RGBA)", "PagingLandScapeOptions::_getAvgColor");
  }
...
void PagingLandScapeOptions::clearTileInfo()
{
  if (!mTileInfoCache.empty()) {
    ...
    //assert (mCurrentMap != "");
    //saveMapInfo();
    std::for_each(mTileInfoCache.begin(), mTileInfoCache.end(), delete_object());
    mTileInfoCache.clear();
  }
}
//-----------------------------------------------------------------------
PagingLandScapeTileInfo *PagingLandScapeOptions::getTileInfo(const uint pageX, const uint pageZ, const uint tileX, const uint tileZ)
{
  ...
  while (q != qend) {
    t = *q;
    if (pageX == t->mPageX && pageZ == t->mPageZ && tileX == t->mTileX && tileZ == t->mTileZ) {
      return t;
    }
    ++q;
  }
...
void PagingLandScapeOptions::saveMapInfo()
{
  if (mUseLodMapCache) {
    ...
    FileInfoListPtr finfo = ResourceGroupManager::getSingleton().findResourceFileInfo(groupName, fInfoName);
    FileInfoList::iterator it = finfo->begin();
    if (it != finfo->end()) {
