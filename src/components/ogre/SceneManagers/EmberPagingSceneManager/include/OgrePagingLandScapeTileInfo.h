...
{
public:
  PagingLandScapeTileInfo(const uint pageX, const uint pageZ, const uint tileX, const uint tileZ) :
      mMinLevelDistSqr(0), mPageX(pageX), mPageZ(pageZ), mTileX(tileX), mTileZ(tileZ)
  {
  }
