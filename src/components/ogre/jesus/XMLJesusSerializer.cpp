...
void XMLJesusSerializer::readAttachPairs(Carpenter::BlockSpec *blockSpec, TiXmlElement* parentElem)
{
  ...
  for (TiXmlElement* elem = parentElem->FirstChildElement(); elem != 0; elem = elem->NextSiblingElement()) {
    tmp = elem->Attribute("name");
    if (!tmp) {
      continue;
    }
...
Carpenter::BluePrint* XMLJesusSerializer::loadBlueprint(const std::string& filename)
{
  ...
  for (TiXmlElement* bbElem = rootElem->FirstChildElement("buildingblocks"); bbElem != 0; bbElem = bbElem->NextSiblingElement()) {
    for (TiXmlElement* smElem = bbElem->FirstChildElement("buildingblock"); smElem != 0; smElem = smElem->NextSiblingElement()) {
  ...
  for (TiXmlElement* bindingsElem = rootElem->FirstChildElement("bindings"); bindingsElem != 0; bindingsElem = bindingsElem->NextSiblingElement()) {
    for (TiXmlElement* smElem = bindingsElem->FirstChildElement("binding"); smElem != 0; smElem = smElem->NextSiblingElement()) {
...
void XMLJesusSerializer::saveBlueprintToFile(Carpenter::BluePrint* blueprint, const std::string& filename, const std::string& name)
{
  ...
  try {
    ...
    if (!osdir) {
      oslink::directory::mkdir((EmberServices::getSingleton().getConfigService().getHomeDirectory() + "carpenter").c_str());
